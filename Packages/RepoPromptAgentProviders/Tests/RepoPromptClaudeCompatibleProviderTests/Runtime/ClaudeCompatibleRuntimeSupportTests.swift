...
final class ClaudeCompatibleRuntimeSupportTests: XCTestCase {
    ...
    func testProviderCatalogDefaultsExposeStableRawValues() throws {
        ...
        let fable51 = try XCTUnwrap(claude.options.first { $0.rawValue == "claude-fable-5-1" })
        XCTAssertEqual(claude.options.dropFirst().first?.rawValue, "claude-fable-5-1")
        XCTAssertEqual(fable51.displayName, "Fable 5.1")
        XCTAssertEqual(fable51.supportedEffortLevels, ["low", "medium", "high", "max", "xhigh"])
        ...
        XCTAssertTrue(expandedClaude.options.contains { $0.rawValue == "claude-fable-5-1:xhigh" })
        ...
        XCTAssertEqual(ClaudeCompatibleHeadlessRuntime.runtimeModelParam("claude-fable-5-1:xhigh"), "claude-fable-5-1")
