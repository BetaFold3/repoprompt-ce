        ...
        private func loadPersistedCodexFixtureBashReplay(
            ...
            let replayableEvents = events.filter { event in
                ...
                case .toolCall, .commandExecutionRunning:
                    true
                case .toolResult, .turnStarted, .turnCompleted, .assistantDelta, .canonicalAssistantDelta, .assistantCompleted, .reasoningDelta, .reasoningCompleted, .tokenUsage, .usageObservation, .modelRerouted, .contextCompacted, .approvalRequest, .permissionsRequest, .requestUserInput, .mcpElicitationRequest, .serverRequestIssue, .livenessActivity, .errorNotification, .error, .system:
                    false
                }
            }
