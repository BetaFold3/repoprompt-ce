...
enum AgentModeMCPToolPolicy {
    ...
    Consult an Oracle model using the current repository evidence. Resolve exact selectable preset names or UUIDs with oracle_utils op=models. For independent opinions, use one consultations batch with an explicit model per lane, or separate single sends with new_chat:true when independent admission is required. Continue completed lanes with their returned chat_id and new_chat:false; the lane keeps its own model preset, so model can be omitted, and the call fails rather than switching models if that preset is no longer usable.
