...
final class MCPOracleToolProvider: MCPWindowToolProviding {
    ...
    private func askOracleTool() -> Tool {
        runtime.tool(
            ...
            `op` defaults to `send`. Single sends, `consultations` batches, and `op:"wait"` are bounded observations: routinely omit `timeout_seconds` to use the parent-family automatic wait, or pass `0` to poll. A timeout or steering wake returns pending operation handles while the same Oracle queries keep running. Pending may include advisory sampled `progress`: response character count and observed stream-activity age for bound work, or a zero-based FIFO queue position for queued batch lanes. It contains no response text and never proves completion, failure, provider liveness, cache state, or TTL. Pending is normal: never resend the question. Resume with `op:"wait"` and the returned handles; after compaction, omit `operation_ids` to collect every owned undelivered operation in the current tab, including more than 16. After a steering wake, respond to the user first, then resume waiting. A wait or transport heartbeat does not issue a provider-model request and does not warm a prompt cache.
            ...
            `consultations` accepts 1...16 independent new-chat lanes and atomically returns one operation receipt per lane in stable input order. The app queues excess lanes in a per-tab FIFO behind the authoritative 2-stream Oracle cap; existing single sends and legacy `oracle_send` streams occupy that cap and are never queued. Batch responses use the same `results` + `wait` + `wait_policy` envelope as `op:"wait"`; each lane keeps its `index`, and pending lanes may report `queued`, `starting`, `streaming`, or `cancelling`. Resume or cancel lanes by operation ID. Batch `request_id` is unsupported: recover accepted work with `op:"wait"` rather than resending. At most 32 nonterminal Oracle operations may exist per tab across all callers and singles/batches. Optional `require_distinct:true` rejects the whole batch before admission when two items resolve to the same preset. Continue completed lanes with their returned `chat_id`. Under parallel use, always pass an explicit `chat_id` to `oracle_chat_log`.
            ...
            inputSchema: .object(
                properties: [
                    ...
                    "timeout_seconds": .number(
                        description: "Single send, consultations batch, or wait observation bound in seconds, 0...14400. Omit for the parent-family automatic wait; zero polls. A timeout returns pending and never stops the Oracle. Rejected with cancel."
                    ),
                    "request_id": .string(
                    ...
                    "consultations": .array(
                        description: "Bounded resumable batch of 1...16 independent new-chat Oracle consultations. Valid only for send (op omitted or send); mutually exclusive with the single-send parameter set, operation_ids, and request_id. Returns stable indexed lane receipts in the shared results/wait envelope, queues excess lanes behind actual per-tab Oracle stream capacity, and supports later wait/cancel by operation ID.",
                        items: .object(
                            properties: [
                                "message": .string(
