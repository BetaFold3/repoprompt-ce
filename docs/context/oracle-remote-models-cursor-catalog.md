...
- Stable OMP menus always use hierarchical exact-wire projection, independent of the OpenCode-only `groupOpenCode` flag. On thinking-enabled destinations, each valid exact-wire OMP model leaf is a submenu whose children begin with Default and then project that model's capability rows. Their item closures rebuild the menu on every open from live state. The literal provider-default placeholder remains an action leaf, and role-default or other no-owner destinations remain model-only leaves.
...
- The Agent Models profile, resolved at global or workspace scope, is the Context Builder agent-thinking authority across its settings, sidebar, and MCP toolbar surfaces.
...
- The Agent Models fix added six explicit post-commit capability-probe sites—three Context Builder surfaces, two role-default surfaces, and the Quick Model Picker HUD commit—in addition to existing picker commits; probes remain out of shared menu helpers.
- Role defaults deliberately have no thinking destination because no per-role thinking owner exists.
