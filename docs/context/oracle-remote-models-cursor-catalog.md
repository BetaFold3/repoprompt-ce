...
- The Agent Models fix added six explicit post-commit capability-probe sites—three Context Builder surfaces, two role-default surfaces, and the Quick Model Picker HUD commit—in addition to existing picker commits; probes remain out of shared menu helpers. Both role-default surfaces attach a stable per-role thinking destination to every accessory-eligible valid exact-wire OMP leaf, even when the role currently resolves to another provider; a thinking child commits its exact model before persisting the role map, while effort-encoded leaves remain terminal.
