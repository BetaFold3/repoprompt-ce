...
- Stable OMP menus always use hierarchical exact-wire projection, independent of the OpenCode-only `groupOpenCode` flag. A corroborated effort/fast family is already the model-and-effort chooser, so every leaf inside that family is a terminal exact-model action and never receives a second thinking submenu. A standalone valid exact-wire leaf remains a thinking submenu on destinations with the accessory; its children begin with Default and then project that model's capability rows, rebuilding from live state on every open. The literal provider-default placeholder remains an action leaf; role-default rows use their own per-role destination, while genuinely model-only destinations remain action leaves. This family-versus-standalone rule is identical across the SwiftUI Agent menu, stable Agent/Context Builder/role menus, and the settings/Oracle/preset builder.
- Thinking intent is destination-owned, not provider-global: each Agent Mode tab/session, Prompt chat/planning/Context Builder destination, model preset, and MCP role default keeps its own capped exact-model map. The local Agent handoff popover is an ephemeral destination initialized from the source session map; its selected map is installed on a newly created OMP destination session after the model selection and before binding/persistence, while non-OMP handoffs receive no OMP assignments. Presets, destinations, roles, or handoffs sharing one wire model may select different thinking values; key absence is Default and sends nothing. Legacy entries for models now classified as family leaves remain stored but are ignored by Oracle/preset/chat and Agent Mode execution while the resolved exact-model catalog retains that family classification; execution fails open only when exact-model/catalog classification is unavailable.
...
- The Agent Models fix added six explicit post-commit capability-probe sites—three Context Builder surfaces, two role-default surfaces, and the Quick Model Picker HUD commit—in addition to existing picker commits; probes remain out of shared menu helpers. Both role-default surfaces attach a stable per-role thinking destination to every standalone valid exact-wire OMP leaf, even when the role currently resolves to another provider; a thinking child commits its exact model before persisting the role map, while family leaves remain terminal.
